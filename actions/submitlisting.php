<?php
    ...
    //Check if valid 
    if (strpos(file_get_contents('/var/cities.geojson'), $_POST['loc']) === false and $cityCheckType == "force") {
        ...
        if ($_POST['have'] != '' and $_POST['want'] != '' and $_POST['loc'] != '' and is_numeric($_POST['amountHave']) and is_numeric($_POST['amountWant']) and $_POST['amountWant'] >= 0 and $_POST['amountHave'] >= 0) {
            if ($_POST['mininc'] != null) {
                if (is_numeric($_POST['mininc'])) {
                    $query = "INSERT INTO offers (poster,have,haveamt,want,wantamt,notes,active,location,creation,aucinc,lastbidder,lastbumped) VALUES(?, ?, ?, ?, ?, ?, 'y', ?, NOW(), ?, ?, NOW())";
                    ...
                    errorOut("Minimum increase must be a number", "danger");
                }
            }
            else {
                $query = "INSERT INTO offers (poster,have,haveamt,want,wantamt,notes,active,location,creation,lastbumped) VALUES(?, ?, ?, ?, ?, ?, 'y', ?, NOW(), NOW())";
                $stmt = mysqli_stmt_init($con);
                mysqli_stmt_prepare($stmt,$query);
            ...
            errorOut("Your input was invalid, please try again", "danger", "/control");
        }
    }
?>